...
export function RatingBtns({ value }) {
    ...
    gap-3 md:gap-4 lg:gap-4 xl:gap-8 2xl:gap-10 `}>
        ...
        <span>
            ...
            <h5 className='font-thin text-[12px] md:text-5xs whitespace-nowrap text-center Regularfont'>
...
export function CourseInfo({ value, Path }) {
    ...
    return <Link to={Path} className='flex flex-col items-center gap-2 text-black no-underline'>
        <h2 className='Blackfont text-[12px]  md:text-3xs h-6'>{CourseName}</h2>
        <p className='max-w-[250px] text-[12px] md:text-4xs Regularfont text-center break-words h-14'>{Description}</p>
    </Link>
}