...
const FIlter = () => {
    ...
    const SortOnChange = (e) => {
        ...
        Socket.emit("SortCourse", { LicenseTypeId: Courses?.LicenseTypeId, FilterValue: e.target.value })
...
function SortBy({ FilterValue, SortOnChange }) {
    return <div className='flex flex-col items-center gap-2 w-full max-w-[300px] px-1 DropDownBorder'>

        <label htmlFor="Sort" className='SemiBold text-3xs'>
            Sort By
        </label>

        <select name="" id="Sort" className='text-3xs pr-5 w-full border-none outline-none'
            value={FilterValue}
...
function FilterComp() {
    return <div className='flex flex-col items-center gap-2 w-full max-w-[300px] px-1 DropDownBorder'>
        ...
        <select name="" id="Filter" className='text-3xs pr-5 w-full border-none outline-none'>
